...
def format_date_column(df, column="date_of_birth_cleaned", output_format="%Y-%m-%d"):
    """Format parsed dates into a consistent ISO 8601 string."""
...
def clean_data(input_path: str, output_path: str, mode: str = "simple",
    ...
    if mode == "simple":
        ...
        if "date_of_birth_cleaned" in df.columns:
            df = df.rename(columns={"date_of_birth_cleaned": "date_of_birth"})
        if "date_of_birth_original" in df.columns:
            df = df.rename(columns={"date_of_birth_original": "date_of_birth_raw"})
